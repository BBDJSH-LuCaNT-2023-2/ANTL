...
TEST_CASE("RegulatorLenstra<ZZ>: Does it work?", "[RegulatorLenstra]") {
  ...
  std::cout << "CASE"
            << std::setw(8 ) << "RESULT"
            << std::setw(9 ) << "CORRECT"
            << std::setw(10) << "COMPUTED"
            << std::setw(7 ) << "DELTA" << std::endl;
  std::cout << std::setw(38) << std::setfill('=') << "" << std::endl;

  for (int i = 0; i < 10; i++) {
    std::cout << std::setfill('0') << std::setw(4) << i + 1 << std::setfill(' ')
              << std::setw(6 ) << computed_correctly[i]
              << std::setw(11) << correct_regulators[i]
              << std::setw(10) << computed_regulators[i]
              << std::setw(7 ) << discriminants[i] << std::endl;
