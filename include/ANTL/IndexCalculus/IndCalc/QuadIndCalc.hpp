...
public:
  ...
  QuadIndCalc<T,R>(IOrder const &quad_order, std::map<std::string, std::string> const &params) : IndCalc<T,R>(quad_order, params) {IndCalc<T,R>::setup_mat(quad_order, params);}
