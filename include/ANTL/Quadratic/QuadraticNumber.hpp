...
namespace ANTL
{
  ...
  template<class T>
    bool
    operator ==<T> (const QuadraticNumber<T> & x, const QuadraticNumber<T> & y);

  template<class T>
    bool
    operator ==<T> (const QuadraticNumber<T> & x, const T & n);

  template<class T>
    bool
    operator ==<T> (const T & n, const QuadraticNumber<T> & x);

  template<class T>
    bool
    operator ==<T> (const QuadraticNumber<T> & x, const QQ<T> & q);

  template<class T>
    bool
    operator ==<T> (const QQ<T> & q, const QuadraticNumber<T> & x);
  ...
  template<class T>
	...
	::set(d);
       ...
       */
      QuadraticNumber (QuadraticOrder & inQO, QQ<T> & q)
      {
	QO = &inQO;
	a = q.getNumerator();
	::clear(b);
	...
	return (x.a == q.getNumerator () && IsZero (x.b) && x.d == q.getDenominator ());
       ...
       */
      friend bool
      operator ==<T> (const QuadraticNumber<T> & x,
		      const QuadraticNumber<T> & y)
      {
	return (x.QO == y.QO && x.a == y.a && x.b = y.b && x.d = y.d);
      }
