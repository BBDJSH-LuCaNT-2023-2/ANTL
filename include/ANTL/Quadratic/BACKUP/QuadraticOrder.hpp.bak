...
#include <ANTL/Quadratic/Reduce/qo_reduce_plain.hpp>
...
namespace ANTL { 
  ...
  template < class T >
  QuadraticOrder<T> randomImaginaryOrder(int size, bool prime = false);

  template < class T >
  QuadraticOrder<T> randomUnusualOrder(int size, bool prime = false);

  template < class T >
  QuadraticOrder<T> randomRealOrder(int size, bool prime = false);
  ...
  public:
    ...
    friend QuadraticOrder<T> randomImaginaryOrder<T> (long size, bool prime = false);
    friend QuadraticOrder<T> randomUnusualOrder<T> (long size, bool prime = false);
    friend QuadraticOrder<T> randomRealOrder<T> (long size, bool prime = false);
...
#define INT_SIZE_QUADRATIC    5000005
  */

  /**
   * @brief compute a list of imaginary quadratic discriminants with
   * L <= |D| <=H
   */

  /*
  void get_Dlist_imag(const ZZ & L, const ZZ & H, vector<ZZ> & Dlist, int init=0);
  void get_Dlist_imag(const long & L, const long & H, vector<long> & Dlist, int init=0);
  */

  /**
   * @brief compute a list of real quadratic discriminants with
   * L <= D <=H
   */
  /*
  void get_Dlist_real(const ZZ & L, const ZZ & H, vector<ZZ> & Dlist, int init=0);
  void get_Dlist_real(const long & L, const long & H, vector<long> & Dlist, int init=0);
  */
