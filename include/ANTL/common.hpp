 ...
 * @brief General-purpose methods to interface with NTL.  All library files
 ...
 */
#define ANTL_END_MACRO_FUNCTION                                                \
  }                                                                            \
  while (0)
...
namespace NTL {
//
// NTL-like methods, for completeness. There exists a "set" and "clear" for most
// (all?) NTL types. The below definitions extend this functionality for other
// types.
...
// Distance updating methods
template <class S>
void update_distance_add(S &distance_new, S const &distance_1,
                         S const &distance_2) {
  distance_new = distance_1 + distance_2;
}

template <class S>
void update_distance_subtract(S &distance_new, S const &distance_1,
                              S const &distance_2) {
  distance_new = distance_1 - distance_2;
}
...
inline float pow(float x, float e) {
  return (float)std::pow((double)x, (double)e);
}
...
template <> inline ZZ CARDINALITY<long>(void) { return to_ZZ(0); }

template <> inline ZZ CARDINALITY<ZZ_p>(void) { return ZZ_p::modulus(); }

template <> inline ZZ CARDINALITY<zz_p>(void) { return to_ZZ(zz_p::modulus()); }

template <> inline ZZ CARDINALITY<ZZ_pX>(void) { return ZZ_p::modulus(); }

template <> inline ZZ CARDINALITY<zz_pX>(void) {
  return to_ZZ(zz_p::modulus());
}

template <> inline ZZ CARDINALITY<ZZ_pE>(void) { return ZZ_pE::cardinality(); }

template <> inline ZZ CARDINALITY<zz_pE>(void) { return zz_pE::cardinality(); }

template <> inline ZZ CARDINALITY<ZZ_pEX>(void) { return ZZ_pE::cardinality(); }

template <> inline ZZ CARDINALITY<zz_pEX>(void) { return zz_pE::cardinality(); }

template <> inline ZZ CARDINALITY<GF2E>(void) { return GF2E::cardinality(); }

template <> inline ZZ CARDINALITY<GF2EX>(void) { return GF2E::cardinality(); }
...
template <> inline double to<double>(const quad_float &a) {
  return to_double(a);
}
 ...
 * representation of p.
 */
template <class T> ZZ eval_poly(const T &p, const ZZ &q);
...
/* Jacobi functions - no preconditions on a and n */
// long Jacobi(const ZZ & a, const ZZ & n);
// long Jacobi(const long & a, const long & n) { return long(Jacobi( to_ZZ(a),
// to_ZZ(n))); }
long Jacobi(const ZZ_pX &a, const ZZ_pX &n);
long Jacobi(const zz_pX &a, const zz_pX &n);
long Jacobi(const ZZ_pEX &a, const ZZ_pEX &n);
 ...
 */
template <class T> long ressol(T &x, const T &a, const T &p);
