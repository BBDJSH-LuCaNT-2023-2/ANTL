...
template < class T >
void 
ExponentiationL2R<T>::initialize(const T &A)
{
  // compute A^-1
  inv(Ainv,A);
...
template < class T > 
void 
ExponentiationL2R<T>::power (T &C, const T &A, const ZZ & n)
{
  ...
  for(j = m; j >= 0; j--){
    i = (b << 3) + (bit(n,j) << 2) + (bit(n,j-1) << 1) + bit(n,j-2);  //if j-1 or j-2 is less than zero, bit returns 0
