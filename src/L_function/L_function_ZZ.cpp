/**
 * @file L_function_ZZ.cpp
 * @author Leonard Nooy
 * @version $Header$
 *
 * This file contains the ZZ specialization for the L_function class
 */
...
namespace ANTL
{
  ...
  ******************************************************************************************/
  ...
  /*
   * Function: L_function<ZZ>::init
   * Description: This function must be called before using the rest of the L_function class
   * Inputs: const ZZ & x - The conductor of the calculation to preform
  ...
  {
    /* set the mode of the L_function. */
    if (m == QUADRATIC_MODE || m == QUARTIC_MODE)
      mode = m;
    else
      {
	cerr << "Init: Unrecognized Mode, Defaulting to Quadratic Mode." <<
	  endl;
	mode = QUADRATIC_MODE;
      }
  ...
  ********************************************************************************************/
   ...
   * Inputs:      L_function<ZZ>& lfunc - the L-function to compute
  ...
  template <class TYPE> CC <RR> approximateL0_Quartic_impl (L_function<ZZ>& lfunc, long terms)
  {
    ...
    for (i = 1; i < terms; i++)
	  ...
	  {
#ifdef USE_CHI2
            CHI = to< CC<TYPE> >(lfunc.Chi.quartic_long(i));
#else
            CHI = to< CC<TYPE> >(lfunc.Chi.quartic_table_long(i));
#endif
    ...
    if (diff1 > 0.015 && diff2 > 0.015)
      {
        cerr <<
	  "Error: L1 and L2 both are not close to being integers! Please run with more terms and/or more precision."
	     << endl;
    ...
    lfunc.L0_result.assign (round (lfunc.L0_result.real ()),
			    round (lfunc.L0_result.imaginary ()));

#ifdef EXTRA
    cerr << "L0 = " << lfunc.L0_result << endl;
#endif
  ...
  /*
   * Function: L_function<ZZ>::approximateL0_Quartic
  ...
  template <class TYPE> RR approximateL0_ImaginaryNumberField_impl(L_function<ZZ>& lfunc, long terms)
  {
    ...
    for (i = 1; i < terms; i++)
      {
	CHI = lfunc.Chi.quadratic_long(i);
  ...
  /*
   * Function: L_function<ZZ>::approximateL0_RealNumberField
   * Description: This function is simply a wrapper for the calculate_optimal_n and approximateL0(long n)
   * Inputs: double acc - The required accuracy
   * Outputs: CC - The result of the calculation
   */
  ...
  *******************************************************************************************/
  ...
  template <class TYPE> RR approximateL1_Quartic_impl(L_function<ZZ>& lfunc, double acc)
  {
    ...
    P = lfunc.primes.next ();		// lfunc.primes start at 3
    while (1)
      {
        P2 = to<TYPE>(P);
        P2 *= P2;
	  ...
	  if (P < 500) {
	    cout << "\ni = " << P << ", X = i,-i" << endl;
	    cout << "L = " << to_double(sqrt(E / (to_RR(lfunc.Delta) / (to_RR(2)*lfunc.PI*lfunc.PI)))) << endl;
	  }
#endif
        }
        else if (kron == 1)
	  {
            X = lfunc.Chi.quartic_long (P);
            if (X.is_one ())
	      term = P2 / (P2 - (P << 1) + 1);
            else
	      term = P2 / (P2 + (P << 1) + 1);
	  ...
	  term = to<TYPE>(1);
        ...
        nE = E*term;
        if (abs(Epref*to_RR(nE-E)) < acc)  break;

        E = nE;
#ifdef EXTRA
        cout << "\nP = " << P << " - " << Epref*to_RR(E) << endl;
        cout << "L = " << to_double(sqrt(E / (to_RR(lfunc.Delta) / (to_RR(2)*lfunc.PI*lfunc.PI)))) << endl;
#endif
        P = lfunc.primes.next ();
      }
  ...
  template <class TYPE> RR approximateL1Bach_Quartic_impl(L_function<ZZ>& lfunc, long terms)
  {
    ...
    // compute partial product  p < Q
    E = log(to<TYPE>(2) / sqrt(to<TYPE>(5)));     // assuming p = 5 mod 8
    lfunc.primes.reset(3);
    P = lfunc.primes.next();		// primes start at 2
    while (P < Q)
      {
        //d = Delta % to_ZZ(P);   // change the negative delta to a positive delta
        dP = to<TYPE>(P);
	  ...
	  if (P < 500) {
	    cout << "\ni = " << P << ", X = i,-i" << endl;
	    cout << "L = " << to_double(exp(to_RR(E))) << endl;
	  }
#endif
        }
        else if (kron == 1)
   ...
   *              number field of quadratic order. The descriminante is
   *              represented by a ZZ, and thus has no restriction.
...
template <> RR L_function < ZZ >::approximateL1 (double acc) {
  ...
  ********************************************************************************************/
  ...
  /*
   * Function: L_function<ZZ>::approximateL2
  ...
  ********************************************************************************************/
  ...
  /*
   * Function: L_function<ZZ>::approximateL
  ...
  /*
   * Function: L_function<ZZ>::approximate
  ...
  ******************************************************************************************/
  ...
  template <>
  RR L_function < ZZ >::approximateL0_RealNumberField_table(double acc)
  {
    RR L0;
  ...
  template <class TYPE> RR approximateL0_RealNumberField_table_impl(L_function<ZZ>& lfunc, long n)
  {
    ...
    D2 = trunc_long(lfunc.Delta,1);
    D8 = trunc_long(lfunc.Delta,3);
  ...
  template <>
  RR L_function < ZZ >::approximateL0_ImaginaryNumberField_table(double acc)
  {
    RR L0;
  ...
  template <class TYPE> RR approximateL0_ImaginaryNumberField_table_impl(L_function<ZZ>& lfunc, long n)
  {
    long int i,Di;	      // loop var
    ...
    D2 = IsOdd(lfunc.Delta);
    D8 = 8 - trunc_long(lfunc.Delta,3);
  ...
  /*
   * Function: L_function<ZZ>::approximateL1_table
   * Description: This function calculates the value of L1 using eric bach's method
   *              of truncated euler terms. It used doubles for internal calculations
  ...
  template <> RR L_function < ZZ >::approximateL1_table ()
  {
    ...
    // compute p=2 term
    long m8 = trunc_long(Delta,3);
    if (Delta < 0)
      m8 = (8 - m8);
    if (m8 == 1)
      E = log( (double) 2);
    else if (m8 == 5)
      E = log( (double) 2 / (double) 3);
    else
      clear(E);
