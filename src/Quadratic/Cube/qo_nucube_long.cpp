...
template <>
void 
qo_nucube<long>::init(const long & Din, const long & hin, long gin)
{
  qo_cube<long>::init(Din,hin,0);
  SQRT_DELTA = FloorTolong(sqrt(abs(to_RR(Din))));
}


template <> 
void
qo_nucube<long>::cube(QuadraticIdealBase<long> &C, const QuadraticIdealBase<long> &A)
{
