...
void QuadIndCalc<T,R>::compute_fac_base(IOrder const &quad_order, std::map<std::string, std::string> const &params) {
  ...
  if ( params.find(fb_size_str) == params.end() ) {
    ...
    IndCalc<T,R>::factor_base = QuadFactorBase<T>(fb_size);
...
void QuadIndCalc<T,R>::compute_relations(IOrder const &quad_order, std::map<std::string, std::string> const &params) {
  ...
  if ( params.find(num_relations_str) == params.end() ) {
    ...
    IndCalc<T,R>::relation_generator = QuadRelationGenerator<T>(quad_order, params, IndCalc<T,R>::factor_base);
...
void QuadIndCalc<T,R>::compute_mat(IOrder const &quad_order, std::map<std::string, std::string> const &params) {
  ...
  IndCalc<T,R>::rels_mat.SetDims(IndCalc<T,R>::cg_relations.size(), IndCalc<T,R>::factor_base.size());
