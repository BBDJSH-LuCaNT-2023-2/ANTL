/**
 * @file DExponentiationIL_impl.hpp
 * @author Michael Jacobson/Reginald Lybbert
 * @brief generic implementation of templated methods from DExponentiationIL class
 */
...
template < class T >
void 
DExponentiationIL<T>::initialize(const T &A, const T&B, const ZZ &m, const ZZ & n, const short w0, const short w1)
{
  ...
  precompA = new T[lenm];
  precompAInv = new T[lenm];
  precompB = new T[lenn];
  precompBInv = new T[lenn];
...
template < class T > 
void 
DExponentiationIL<T>::power (T &C, const T &A, const T &B, const ZZ & m, const ZZ & n)
{
  ...
  for (register long j = em.size()-2; j >= 0; --j)
    {
      sqr(C, C);
      if (em.at(j) > 0){
        x = (em.at(j) - 1)/2;
